...
function validateAndFixClassChain(classChain) {
  ...
  let chain = classChain.replace('-ios class chain:', '');
  ...
  if (predicateMatches) {
    ...
    for (let i = 0; i < predicateMatches.length; i++) {
      ...
      pred = pred.replace(/\b([a-zA-Z0-9_-]+)\s*(==|!=)\s*['"]([^'"]+)['"]/gi, (match, attr, operator, val) => {
        const booleanAttrs = ['enabled', 'visible', 'accessible', 'selected', 'hittable', 'focused', 'exists'];
        if (booleanAttrs.includes(attr.toLowerCase())) {
  ...
  if (/==\s*['"][^'"]*\*/.test(chain)) {
    return fail('Invalid wildcard usage with == (use LIKE/MATCHES instead)');
  }

  return {
    valid: true,
    fixedLocator: `-ios class chain:${chain}`,
    reason: null,
  };
}
