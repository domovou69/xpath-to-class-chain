...
function isLikelyIosLocator(str) {
  ...
  const hasIosSignal =
    /XCUIElementType[A-Za-z]+/.test(str) || // native element type
    /@(?:name|label|value|type|text|enabled|visible|accessible|selected|focused|hittable)\b/.test(str) || // known attrs
    /\b(?:contains|starts-with|ends-with)\s*\(/i.test(str) || // string functions
    /\[[^\]]*\]/.test(str); // any predicate at all (e.g. //Type[1])
  if (!hasIosSignal) return false;

  // Cheap structural sanity check: balanced predicate brackets. Anything
  // malformed past this still gets caught by the validator downstream.
...
function processFile(filePath, opts, records, stats) {
  ...
  // Any quoted string whose content starts like a locator: `//` (XPath) or
  // `-ios class chain:` (existing). The leading `ios:`/`android:` key (if any)
  // sits OUTSIDE the match and is left untouched, so this works whether or not
  // consumers tag their locators with a platform prefix.
  // isLikelyIosLocator() is what keeps us from rewriting URLs/comments.
  const strictRegex = /(['"`])((?:\/\/|-ios class chain:)(?:\\.|(?!\1).)*)\1/g;
  ...
  const updatedContent = content.replace(strictRegex, (match, quote, locator, offset) => {
    ...
    const preceding = content.slice(Math.max(0, offset - 60), offset);
    const isAndroidKey = /(?<![a-zA-Z_$0-9])['"]?android['"]?\s*:\s*$/.test(preceding);
    if (isAndroidKey || locator.includes('android.')) {
