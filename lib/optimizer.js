...
//   Rule 6:  converter.js merges multiple [@a][@b] predicate blocks into one AND
...
function conditionCost(cond) {
  for (const [op, cost] of Object.entries(OPERATOR_COST)) {
    ...
    if (new RegExp(`\\b${op}(?:\\[c[di]?\\])?\\b`, 'i').test(cond)) return cost;
  }
